...
**LoL Coach** is an AI-assisted item advisor for *League of Legends*. The system reads real-time game data from the local Riot Games Live Client API during an active match, detects game-relevant events, and provides the player with context-aware item recommendations — either rule-based (heuristic) or enriched with a natural-language explanation generated by an LLM.
...
| F3 | Rule-based heuristic recommendations with no external dependency |
| F4 | Optional LLM enrichment with a cooldown mechanism to control API costs |
...
|----------|------------------|------|
| 1 | **Reliability / Offline Operation** | Recommendations are available even without internet access (heuristic mode) |
| 2 | **Performance Efficiency** | Heuristic recommendation delivered in < 5 ms; LLM explanation in < 3 s |
...
| `ItemRecommendation` (heuristic + LLM) | WebSocket :8765 | Outbound from Bridge |
...
| How are recommendations generated? | Heuristic-first, LLM-optional | Heuristic is immediately available (< 5 ms), requires no internet; LLM enriches only when needed | Performance, Offline Operation |
...
|---------------|----------------|
| **Bridge** | Data acquisition from the Riot API, event detection, recommendation generation (heuristic + LLM), WebSocket server |
...
- **heuristic.ts**: Computes a `CompProfile` from the enemy team and returns an `ItemRecommendation`.
...
- Fall back silently to the heuristic `reasoning` string on error (no user-visible failure)
...
This scenario describes the most architecturally relevant flow: the Flutter app connects to the Bridge, a game starts, and the player receives both a heuristic and an LLM-generated item recommendation.
...
- The heuristic recommendation and LLM explanation are transmitted as separate `RECOMMENDATION` messages
- The LLM call is optional and does not block the heuristic path

## Scenario: Enemy Purchases Item (Heuristic Only) {#_runtime_scenario_2}

This scenario shows the heuristic path without an LLM call. An enemy purchases an item, the `EventDetector` raises `ITEM_PURCHASED`, and the orchestrator decides based on the active cooldown whether an LLM call is triggered.

![Sequence: Item Purchase (Heuristic Only)](umls/06_seq_itempurchase.svg)
...
**Motivation:** Local execution protects player privacy (no game state data leaves the machine except for optionally configured LLM calls) and enables fully offline operation.
...
| LLM API call | On failure, falls back silently to the heuristic `reasoning` string — no user-visible error |
...
## ADR-001: Heuristic-First with Optional LLM

**Context:** Item recommendations must work without internet access and without a configured API key.

**Decision:** The rule-based heuristic engine is always active and delivers an immediate recommendation. The LLM provider is optional and enriches the recommendation with a natural-language explanation on demand.
...
| (+) | No latency dependency on the LLM for the core feature |
| (+) | Silent fallback on LLM failure (no user-visible error) |
| (−) | Heuristic explanations are less context-aware than LLM explanations |
...
|----|------------------|--------------------|-----------------|-------------------------------|
| Q1 | Reliability | Internet access is lost during a game | Heuristic continues to deliver item recommendations | Heuristic recommendation appears in < 5 ms; no error visible |
| Q2 | Performance Efficiency | `GAME_STARTED` event is detected | Heuristic recommendation is broadcast immediately | < 5 ms after event detection |
...
| S6 | **No role-awareness in the heuristic** | Low | Medium | Recommendations do not differentiate between support, carry, and jungler roles. An ADC receives the same suggestions as a support. |
...
| **Heuristic** | Rule-based item recommendation system with no external API dependency |
