import type { AllGameData, Player } from "../types.js";
import { makePlayer, makeActivePlayer, makeItem } from "./fixtures.js";

// Thornmail — anti-heal/armor item, identifiable by heuristic
const THORNMAIL = makeItem({ itemID: 3076, displayName: "Thornmail", price: 2700, slot: 0 });

// Builds one raw game snapshot as the Riot Live Client API would return it.
function snap(
  gameTime: number,
  localOverrides: Partial<Player>,
  enemyItemMap: Record<string, ReturnType<typeof makeItem>[]>,
  gold: number,
): AllGameData {
  const mk = (name: string, champion: string, team: "ORDER" | "CHAOS") =>
    makePlayer({
      summonerName: name,
      team,
 ...
 * (MAX_POLL_FAILURES = 3).
 */
export function buildGameScenario(): AllGameData[] {
