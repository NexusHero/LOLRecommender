...
const ddragonService_1 = require("../ddragonService");
...
describe("DDragonService.init", () => {
    it("init_NoCacheOnDisk_FetchesVersionAndItems", async () => {
        makeFullFetchMock();
        const svc = new ddragonService_1.DDragonService();
        await svc.init();
    ...
    it("init_CacheExistsForCurrentVersion_DoesNotFetchNetwork", async () => {
        ...
        expect(global.fetch.mock.calls[0][0]).toContain("versions.json");
        expect(svc.getItemInfo(3102)?.name).toBe("Banshee's Veil");
    });
    it("init_CacheExistsForOlderVersion_FetchesFreshData", async () => {
        ...
        fs.readFile.mockResolvedValue(JSON.stringify(staleCache));
        makeFullFetchMock();
        const svc = new ddragonService_1.DDragonService();
        await svc.init();
    ...
    it("init_CalledConcurrently_OnlyFetchesOnce", async () => {
        ...
        const versionCalls = global.fetch.mock.calls.filter((c) => c[0].includes("versions.json"));
