...
exports.buildCompProfile = buildCompProfile;
exports.buildHeuristicStrategy = buildHeuristicStrategy;
exports.getHeuristicRecommendations = getHeuristicRecommendations;
const stateMinifier_js_1 = require("./stateMinifier.js");
const champions_json_1 = __importDefault(require("./data/champions.json"));
const items_json_1 = __importDefault(require("./data/items.json"));
const AP_CHAMPIONS = new Set(champions_json_1.default.ap);
...
function buildCompProfile(enemies) {
    ...
    for (const enemy of enemies) {
        if (AP_CHAMPIONS.has(enemy.championName))
            apCount++;
        if (CC_CHAMPIONS.has(enemy.championName))
            ccCount++;
        if (HEALER_CHAMPIONS.has(enemy.championName))
            healCount++;
    }
...
function buildHeuristicStrategy(state, profile, myChampion) {
    ...
    const winCondition = isAhead ? "early" : isApChamp ? "late" : "mid";
    let summary;
    let immediateAction;
    let lateGamePlan;
    if (isAhead) {
...
function formatReasoning(profile) {
    const parts = [];
    if (profile.apRatio >= apRatioForBanshee)
        parts.push(`AP-heavy (${Math.round(profile.apRatio * 100)}%)`);
    else if (profile.adRatio >= 0.6)
        parts.push(`AD-heavy (${Math.round(profile.adRatio * 100)}%)`);
    else
        parts.push("Mixed damage");
    if (profile.ccScore >= 2)
        parts.push(`${profile.ccScore}x CC`);
    if (profile.healScore >= 1)
        parts.push(`${profile.healScore}x healing`);
