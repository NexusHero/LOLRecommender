...
exports.SYSTEM_PROMPT = `You are an experienced League of Legends coach giving live in-game advice.
...
const LlmResponseSchema = zod_1.z.object({
    itemReasoning: zod_1.z.string().optional(),
    situationalItems: zod_1.z.array(zod_1.z.object({
        id: zod_1.z.number().catch(0),
        name: zod_1.z.string().catch(""),
        reason: zod_1.z.string().catch("")
    })).optional(),
    strategy: zod_1.z.object({
        winCondition: zod_1.z.enum(["early", "mid", "late"]).optional(),
        summary: zod_1.z.string().optional(),
        immediateAction: zod_1.z.string().optional(),
        lateGamePlan: zod_1.z.string().optional(),
        laneMatchupAnalysis: zod_1.z.string().optional(),
        counterPlay: zod_1.z.string().optional(),
    }).optional()
});
function buildRoleContext(state, role) {
    switch (role) {
        case "UTILITY": {
        ...
        case "JUNGLE": {
            const losingLanes = state.allies.filter((a) => a.scores.deaths > a.scores.kills + a.scores.assists);
...
async function formatAbilities(championName) {
    const abilities = await ddragonService_js_1.ddragon.getChampionAbilities(championName);
    if (!abilities)
        return "";
    return ` | Abilities: Q=${abilities.q}, W=${abilities.w}, E=${abilities.e}, R=${abilities.r}`;
}
function findLaneOpponent(state, myPos) {
    if (myPos === "UNKNOWN" || myPos === "")
        return undefined;
...
async function formatOpponent(primaryOpponent, myPos) {
    if (!primaryOpponent)
        return "Opponent: unknown (position data unavailable)";
    const opponentAbilityStr = await formatAbilities(primaryOpponent.championName);
    const tags = ddragonService_js_1.ddragon.getChampionTags(primaryOpponent.championName);
...
function formatEnemiesWithTags(enemies) {
    if (enemies.length === 0)
        return "None";
    return enemies.map((e) => {
        const tags = ddragonService_js_1.ddragon.getChampionTags(e.championName);
        const tagStr = tags.length ? ` [${tags.join(", ")}]` : "";
...
async function buildUserPrompt(state) {
    ...
    const myTags = ddragonService_js_1.ddragon.getChampionTags(state.localPlayer.championName);
...
function parseAnalysisResponse(raw) {
    try {
        ...
        if (!parsed.success) {
            logger_js_1.Logger.warn("[LLM] Failed to parse JSON according to schema:", parsed.error);
            return { reasoning: "", strategy: DEFAULT_STRATEGY };
        }
        const data = parsed.data;
        const situationalItems = (data.situationalItems ?? [])
            .slice(0, 4)
            .map((item) => ({
            id: item.id,
            name: item.name,
 ...
 */
async function createLlmProvider(type, apiKey, model) {
    switch (type) {
        case "claude":
            return new claudeProvider_js_1.ClaudeProvider(apiKey, model);
        case "openai":
            return new openaiProvider_js_1.OpenAiProvider(apiKey, model);
        case "gemini":
            return new geminiProvider_js_1.GeminiProvider(apiKey, model);
        default:
